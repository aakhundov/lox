...
UTEST_F(lox, a_call_of_a_field_holding_a_function_runs_the_field) {
  // a property read and a call written together are one instruction now, and
  // it has to find a field where a plain read would have found one
  ASSERT_TRUE(run(utest_fixture, "class A {} fun f() { return 42; }"
                                 "var a = A(); a.g = f;"
                                 "print a.g();"));
  EXPECT_VALUE_EQ(CLOX_NUMBER(42.0), only_printed(utest_fixture));
}

UTEST_F(lox, a_call_of_a_field_shadows_the_method_of_that_name) {
  ASSERT_TRUE(run(utest_fixture, "class A { m() { return 1; } } fun f() { return 42; }"
                                 "var a = A(); a.m = f;"
                                 "print a.m();"));
  EXPECT_VALUE_EQ(CLOX_NUMBER(42.0), only_printed(utest_fixture));
}

UTEST_F(lox, a_call_of_a_field_holding_a_native_reaches_the_native) {
  ASSERT_TRUE(run(utest_fixture, "class A {} var a = A(); a.n = len;"
                                 "print a.n(\"abcd\");"));
  EXPECT_VALUE_EQ(CLOX_NUMBER(4.0), only_printed(utest_fixture));
}

UTEST_F(lox, a_call_of_a_field_holding_a_class_makes_an_instance) {
  ASSERT_TRUE(run(utest_fixture, "class A {} class B { init(v) { this.v = v; } }"
                                 "var a = A(); a.make = B;"
                                 "print a.make(42).v;"));
  EXPECT_VALUE_EQ(CLOX_NUMBER(42.0), only_printed(utest_fixture));
}

UTEST_F(lox, a_call_of_a_field_holding_a_method_of_another_instance_runs_on_that_one) {
  // the binding carries its own receiver, and the instance the field was read
  // off is not it
  ASSERT_TRUE(run(utest_fixture, "class A { init(v) { this.v = v; } get() { return this.v; } }"
                                 "var a = A(1); var b = A(2);"
                                 "a.f = b.get;"
...
UTEST_F(lox, a_method_calls_another_method_on_this) {
  ASSERT_TRUE(run(utest_fixture, "class A {"
                                 "  init() { this.n = 0; }"
                                 "  bump() { this.n = this.n + 1; }"
                                 "  twice() { this.bump(); this.bump(); }"
                                 "}"
                                 "var a = A(); a.twice();"
                                 "print a.n;"));
  EXPECT_VALUE_EQ(CLOX_NUMBER(2.0), only_printed(utest_fixture));
...
UTEST_F(lox, calling_what_a_method_returned_calls_that_value) {
  ASSERT_TRUE(run(utest_fixture, "fun f() { return 42; }"
                                 "class A { get() { return f; } }"
                                 "print A().get()();"));
  EXPECT_VALUE_EQ(CLOX_NUMBER(42.0), only_printed(utest_fixture));
}

UTEST_F(lox, a_method_call_on_the_result_of_a_call_reaches_the_new_instance) {
  ASSERT_TRUE(run(utest_fixture, "class A { init(v) { this.v = v; } get() { return this.v; } }"
                                 "fun make(v) { return A(v); }"
                                 "print make(42).get();"));
  EXPECT_VALUE_EQ(CLOX_NUMBER(42.0), only_printed(utest_fixture));
}

UTEST_F(lox, calling_a_property_of_something_that_is_not_an_instance_is_a_runtime_error) {
  // reading a property and calling one are separate instructions, and each
  // says what it was looking for
  ASSERT_FALSE(run(utest_fixture, "var n = 1; n.m();"));
  ASSERT_TRUE(utest_fixture->errors.count > 0);
  EXPECT_TRUE(strstr(utest_fixture->errors.messages[0], "only instances have methods") != NULL);
}

UTEST_F(lox, calling_a_property_of_a_class_is_a_runtime_error) {
  ASSERT_FALSE(run(utest_fixture, "class A { m() { return 1; } } A.m();"));
  ASSERT_TRUE(utest_fixture->errors.count > 0);
  EXPECT_TRUE(strstr(utest_fixture->errors.messages[0], "only instances have methods") != NULL);
}
...
UTEST_F(lox, calling_a_field_that_is_not_callable_is_a_runtime_error) {
  ASSERT_FALSE(run(utest_fixture, "class A {} var a = A(); a.f = 1; a.f();"));
  ASSERT_TRUE(utest_fixture->errors.count > 0);
  EXPECT_TRUE(strstr(utest_fixture->errors.messages[0], "call") != NULL);
}

UTEST_F(lox, an_initializer_called_again_by_name_runs_on_the_instance_it_was_reached_through) {
  ASSERT_TRUE(run(utest_fixture, "class A { init(v) { this.v = v; } }"
                                 "var a = A(1); a.init(2);"
                                 "print a.v;"));
  EXPECT_VALUE_EQ(CLOX_NUMBER(2.0), only_printed(utest_fixture));
}

UTEST_F(lox, a_method_call_on_a_capturing_method_survives_a_collection) {
  // the method is a closure the run made, and the class it was recorded on is
  // reachable only through the instance the call is written on
  ASSERT_TRUE(run(utest_fixture, "fun make() {"
                                 "  var v = \"cap\" + \"tured\";"
                                 "  class A { get() { return v; } }"
                                 "  return A();"
                                 "}"
                                 "var a = make();"
                                 "gc();"
