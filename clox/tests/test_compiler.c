...
// The same call written as a method call -- "a.m(1,1,...);" -- so the argument
// list the invoke form counts is measured against the same limit.
static const char *invoke_of_args(char (*buffer)[SOURCE_SIZE], size_t count) {
  size_t written = (size_t)snprintf(*buffer, SOURCE_SIZE, "a.m(");
  for (size_t i = 0; i < count; i++) {
    written +=
        (size_t)snprintf(*buffer + written, SOURCE_SIZE - written, "%s1", (i > 0) ? "," : "");
  }
...
UTEST_F(compiler, a_method_call_is_one_instruction_naming_the_property_and_counting_its_arguments) {
  ASSERT_TRUE(compile(utest_fixture, "a.m(1);"));

  // reading the property and calling what came back are one instruction, so
  // nothing is ever bound to the receiver on the way through
  EXPECT_CODE(&utest_fixture->function->chunk, OP_GET_GLOBAL, 0, OP_CONSTANT, 1, OP_INVOKE, 2, 1,
              OP_POP, OP_RETURN_NIL);
...
UTEST_F(compiler, a_method_call_without_arguments_counts_none) {
  ASSERT_TRUE(compile(utest_fixture, "a.m();"));

  EXPECT_CODE(&utest_fixture->function->chunk, OP_GET_GLOBAL, 0, OP_INVOKE, 1, 0, OP_POP,
              OP_RETURN_NIL);
}

UTEST_F(compiler, a_method_call_evaluates_its_arguments_left_to_right) {
  ASSERT_TRUE(compile(utest_fixture, "a.m(1, 2);"));

  EXPECT_CODE(&utest_fixture->function->chunk, OP_GET_GLOBAL, 0, OP_CONSTANT, 1, OP_CONSTANT, 2,
              OP_INVOKE, 3, 2, OP_POP, OP_RETURN_NIL);
}

UTEST_F(compiler, a_property_that_is_read_and_not_called_is_still_a_get) {
  // the two forms part on the '(' alone: what stands after the name is what
  // decides which instruction the property becomes
  ASSERT_TRUE(compile(utest_fixture, "a.m;"));
...
UTEST_F(compiler, only_the_last_property_of_a_chain_is_called) {
  ASSERT_TRUE(compile(utest_fixture, "a.b.m();"));

  EXPECT_CODE(&utest_fixture->function->chunk, OP_GET_GLOBAL, 0, OP_GET_PROP, 1, OP_INVOKE, 2, 0,
              OP_POP, OP_RETURN_NIL);
}

UTEST_F(compiler, calling_what_a_method_call_returned_is_a_plain_call) {
  ASSERT_TRUE(compile(utest_fixture, "a.m()();"));

  // the second call has no property to name, so it stays the general form
  EXPECT_CODE(&utest_fixture->function->chunk, OP_GET_GLOBAL, 0, OP_INVOKE, 1, 0, OP_CALL, 0,
              OP_POP, OP_RETURN_NIL);
}

UTEST_F(compiler, a_property_set_is_not_turned_into_a_call_by_what_follows_it) {
  // the assignment is matched before the '(' is, so a parenthesised value
  // stays the value being stored
  ASSERT_TRUE(compile(utest_fixture, "a.b = (1);"));
...
UTEST_F(compiler, a_method_call_on_this_reads_the_receiver_slot) {
  ASSERT_TRUE(compile(utest_fixture, "class A { m() { return this.n(); } }"));

  clox_function_t *method = function_constant(&utest_fixture->function->chunk, 1);
  ASSERT_TRUE(method != NULL);
  EXPECT_CODE(&method->chunk, OP_GET_LOCAL, 0, OP_INVOKE, 0, 0, OP_RETURN, OP_RETURN_NIL);
}

UTEST_F(compiler, a_method_call_at_a_long_index_takes_the_long_form) {
  char source[SOURCE_SIZE];
  ASSERT_TRUE(compile(utest_fixture, past_a_byte_of_constants(&source, "a.m(1);")));

  // the tail is the long invoke, its three index bytes and its count, then
  // the pop and the script's own return
  const clox_chunk_t *chunk = &utest_fixture->function->chunk;
  ASSERT_TRUE(chunk->length >= 7);
  EXPECT_EQ((clox_byte_t)OP_INVOKE_LONG, chunk->code[chunk->length - 7]);
...
UTEST_F(compiler, the_widest_argument_list_is_accepted_by_a_method_call) {
  char source[SOURCE_SIZE];

  ASSERT_TRUE(compile(utest_fixture, invoke_of_args(&source, CLOX_MAX_ARITY)));

  const clox_chunk_t *chunk = &utest_fixture->function->chunk;
  // the tail is the invoke, its index and its count, then the pop and the return
  ASSERT_TRUE(chunk->length >= 5);
  EXPECT_EQ((clox_byte_t)CLOX_MAX_ARITY, chunk->code[chunk->length - 3]);
}

UTEST_F(compiler, one_argument_too_many_in_a_method_call_is_reported) {
  char source[SOURCE_SIZE];

  EXPECT_FALSE(compile(utest_fixture, invoke_of_args(&source, CLOX_MAX_ARITY + 1)));

  ASSERT_TRUE(utest_fixture->errors.count > 0);
  EXPECT_TRUE(strstr(utest_fixture->errors.messages[0], "args") != NULL);
}

UTEST_F(compiler, an_argument_list_too_long_is_reported_once_however_far_it_runs_over) {
  // the limit is crossed at one argument and reported there; the arguments
  // after it are the same one mistake, not new ones
  char source[SOURCE_SIZE];

  EXPECT_FALSE(compile(utest_fixture, call_of_args(&source, CLOX_MAX_ARITY + 4)));
