...
static inline bool call_class(clox_vm_t *vm, const clox_class_t *class_, size_t arg_count) {
...
static inline bool call_bound_method(clox_vm_t *vm, const clox_bound_method_t *bm,
                                     size_t arg_count) {
  // put the receiver instance on the stack
...
static inline bool call_value(clox_vm_t *vm, clox_value_t val, size_t arg_count) {
  ...
  if (CLOX_IS_OBJECT(val)) {
    ...
    case OBJ_CLASS:
      return call_class(vm, CLOX_AS_CLASS(val), arg_count);
    case OBJ_FUNCTION:
    ...
    case OBJ_BOUND_METHOD:
      return call_bound_method(vm, CLOX_AS_BOUND_METHOD(val), arg_count);
    case OBJ_STRING:
    case OBJ_UPVALUE:
    case OBJ_INSTANCE:
...
static inline bool invoke_from_class(clox_vm_t *vm, const clox_class_t *class_,
...
static inline bool invoke(clox_vm_t *vm, clox_value_t receiver, const clox_string_t *name,
                          size_t arg_count) {
  if (!CLOX_IS_INSTANCE(receiver)) {
    error(vm, "only instances have methods");
    return false;
  }

  clox_instance_t *instance = CLOX_AS_INSTANCE(receiver);

  clox_value_t field;
  if (clox_table_get(&instance->fields, name, &field)) {
    // if field with (name) is found, call it instead
    *(vm->stack_top - 1 - arg_count) = field;
    return call_value(vm, field, arg_count);
  }

  return invoke_from_class(vm, instance->class_, name, arg_count);
}
